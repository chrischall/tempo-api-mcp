...
- **Build before run**: `dist/` must exist before running the server manually. `npm run build` runs `tsc` (→ `dist/index.js`, the npm bin) then `npm run bundle` = esbuild (→ `dist/bundle.js`, the MCPB entry point, with `dotenv` left external). `--alias:zod/v4=./node_modules/zod/v4/index.cjs` is deliberate insurance, not an accident: without it alltrails-mcp (#119) shipped a standalone bundle whose Zod failed to initialise at runtime. That failure does not reproduce here — a control bundle built without the alias serves every tool and still rejects bad input — but no test covers the break, so the flag stays; it costs roughly a doubled artifact (1.4 MB → 2.3 MB). Don't trim it without reproducing #119 first.
- **Tool registration shape**: tools use `server.registerTool(name, { description, annotations, inputSchema }, handler)` with a full `z.object({ … })` in `inputSchema` — SDK **v2** takes a Zod object where v1 took a raw field object, so a tool copied from a pre-SDK-v2 revision of this repo will not register. Mutating tools should set `annotations.readOnlyHint: false`.
...
- **Search filters must match the spec's `*SearchInput` schema exactly.** Tempo silently ignores unknown body fields on its `/search` endpoints, so an invented filter doesn't 400 — it comes back as an *unfiltered* result set that looks like a working search. `tempo_search_accounts` (`ids`/`keys`/`statuses`/`global`) and `tempo_search_team_memberships` (`teamIds`/`accountIds`/`roleIds`) pin their allowed fields in a `*_SEARCH_FILTERS` tuple; check the schema in [`tempo-openapi.yaml`](https://apidocs.tempo.io/) before adding one.
- **Accounts are addressed two ways**: `GET /4/accounts/{id}` takes a **numeric id**, while `PUT`/`DELETE /4/accounts/{key}` take the **string key**. `tempo_get_account` therefore takes `id`, not `key`; `tempo_search_accounts(keys: [...])` resolves one to the other.
